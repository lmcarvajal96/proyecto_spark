...
trait BatchJob {
  ...
  def run(args: Array[String]): Unit = {
    ...
    val aggBySumMailDF = TotalBytesMail(antennaMetadataDF)
    writeToJdbc(aggBySumAppDF, jdbcUri, aggJdbcTable, jdbcUser, jdbcPassword)
    val aggBySumQuotesDF = userQuotes(antennaMetadataDF)
    writeToJdbc(aggBySumQuotesDF, jdbcUri, aggJdbcTable, jdbcUser, jdbcPassword)
    writeToJdbc(aggBySumMailDF, jdbcUri, aggJdbcTable, jdbcUser, jdbcPassword)
    writeToJdbc(aggBySumAntennaDF, jdbcUri, aggJdbcTable, jdbcUser, jdbcPassword)
