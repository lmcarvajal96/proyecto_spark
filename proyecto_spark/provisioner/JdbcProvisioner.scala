...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      println("Creando la tabla antenna_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS app_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")

      println("Creando la tabla antenna_1h_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS app_1h_sum (app TEXT, date TIMESTAMP, total_bytes BIGINT)")

      println("Creando la tabla antenna_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS email_sum (date TIMESTAMP,email TEXT,  total_bytes BIGINT)")

      println("Creando la tabla antenna_1h_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS email_1h_sum (email TEXT, date TIMESTAMP, total_bytes BIGINT)")

      println("Creando la tabla antenna_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS userQuotes (date TIMESTAMP,email TEXT, fraude TEXT)")

      println("Creando la tabla antenna_1h_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS userQuotes_1h (date TIMESTAMP,email TEXT, fraude TEXT)")



      println("Creando la tabla antenna_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS antenna_sum (date TIMESTAMP,antenna_id TEXT,  total_bytes BIGINT)")

      println("Creando la tabla antenna_1h_sum (date TIMESTAMP,app TEXT,  total_bytes BIGINT)")
      statement.execute("CREATE TABLE IF NOT EXISTS antenna_1h_sum (antenna_id TEXT, date TIMESTAMP, total_bytes BIGINT)")
